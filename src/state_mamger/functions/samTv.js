...
import {
  startStreamRecordingPath,
  stopStreamRecordingPath,
} from "../../utils/networks/endpoints";
import {
  deleteAllAudience,
  deleteAllChats,
  deleteAllLikes,
...
export const startRecording = () => async (dispatch) => {
  ...
  axios
    .post(startStreamRecordingPath, data, config)
    .then((res) => {
...
export const endRecording = () => (dispatch) => {
  ...
  axios
    .post(stopStreamRecordingPath, data, config)
    .then((res) => {
      console.log(res);
      deleteRecordingResourceId();
      deleteRecordingSid();
      ...
      tracks[0].setEnabled(false).then(() => {
        ...
        db.collection("samTv")
          ...
          .then(() => {
            ...
            deleteStreamUid();
            pushNotification(
              "Prophet Sampson Amoateng's livestream of Spiritual Breakfast just sent ended on SAM TV. ",
  ...
  getVideoToken().then((token) => {
    if (!tracks && !ready) {
      notification.error({
        ...
        description:
          "Make sure ur deivce is conneted to a camera and a microphone",
      });
    } else {
        ...
        .then((uid) => {
          ...
          if (ready && tracks) {
            client.setClientRole("host").then(() => {
              tracks.setVideoProfile("720p_6");
              client.publish(tracks).then((res) => {
                pushNotification(
                  "Prophet Sampson Amoateng is live on SAM TV with Spiritual Breakfast.",
                  "Join us",
                  "sam_tv"
                );
